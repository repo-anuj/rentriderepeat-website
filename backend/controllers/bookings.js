...
exports.updateBookingStatus = async (req, res, next) => {
  try {
    ...
    if (
      !status ||
      !["pending", "confirmed", "cancelled", "completed", "rejected"].includes(
        status
      )
    ) {
      return res.status(400).json({
...
exports.processPayment = async (req, res, next) => {
  try {
    ...
    // Process payment
    const booking = await BookingService.processPayment(id, paymentDetails);

    // Send payment confirmation email
    try {
      // Get bike details
      const bike = await BikeService.getBikeById(booking.bike);

      const message = `
        Your payment has been processed successfully!

        Booking Details:
        - Bike: ${bike.name} ${bike.brand} ${bike.model}
        ...
        Payment Details:
        - Payment Method: ${booking.paymentDetails.paymentMethod}
        - Payment ID: ${booking.paymentDetails.paymentId || "N/A"}
        - Amount Paid: ₹${booking.paymentDetails.amount}
    ...
    return successResponse(res, 200, "Payment processed successfully", booking);
  } catch (err) {
    next(err);
  }
};
